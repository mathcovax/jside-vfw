...
export default class Page{
    ...
    constructor(name, html){
        if(!name) throw "Page : Page need a name."
    ...
    render(root={extComponent: {}}){
        ...
        while((page.document.querySelector("div[data-mc]") && module) || page.document.querySelector("div[data-ec]")){
            ...
            if(module){
                for(const component of page.document.querySelectorAll("div[data-mc]")){
                    if(component.dataset.mc.endsWith(".*")){
                        ...
                        let div = page.document.createElement("div")
                        if(this.module.component[component.dataset.mc])div.innerHTML = this.module.component[component.dataset.mc].render(component.innerHTML === ""? false : component.innerHTML)
            ...
            for(const component of page.document.querySelectorAll("div[data-ec]")){
                if(component.dataset.ec.endsWith(".*")){
                    let innerHTML = component.innerHTML
                    component.innerHTML = ""
                    for(const key in this.module?.root?.extComponent || root.extComponent){
                    ...
                    let div = page.document.createElement("div")
                    if((this.module?.root?.extComponent || root.extComponent)[component.dataset.ec])div.innerHTML = (this.module?.root?.extComponent || root.extComponent)[component.dataset.ec].render(component.innerHTML === ""? false : component.innerHTML)
        ...
        try{
            for(const style of page.document.querySelectorAll("style[type='text/scss']")){
                style.textContent = sass.compileString(style.innerHTML, {style: "compressed"}).css.toString()
                style.type = "text/css"
            }
        }
        catch(e){
        ...
        if(module){
            let rootIndex = ToWindow(this.module.root.index).window
            rootIndex.document.body.dataset.jsideScoped = this.module.name + "/" + this.name
            ...
            html = rootIndex.document.documentElement.outerHTML
            rootIndex.close()
            ...
            html = page.document.documentElement.outerHTML
        }
