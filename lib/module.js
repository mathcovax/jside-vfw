...
export default class Module{
    ...
    async httpRequest(req, res){
        switch (req.method){
            ...
            case "GET":
                ...
                switch (await (async () => {
                    try{
                        return await this.#modulesGET[req.baseUrl.join("/")].httpAcces(req, res, {json: this.json, gson: this.root.json})
                    }catch(e){
    ...
    getHttpAcces(path, httpAcces){
        ...
        this.#modulesGET[this.name + path].httpAcces = (async (req, res, short) => {return await httpAcces(req, res, async () => {return await this.#httpAcces(req , res)}, short)})
        return this
    }
     ...
     */
    getPage(pageName){
        if(!this.#page[pageName]) throw "Module : page '" + pageName + "' don't existe in module '" + this.name + "'."
