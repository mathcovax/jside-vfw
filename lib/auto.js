...
export default async function auto(path, root){
    ...
    module.json = JSON.parse(fs.readFileSync(rootPath + "/$/$.json"))
    if(!module.json.pageRender?.["$"]){
        module.json.pageRender["$"] = {pageTitle: "page name"}
        fs.writeFileSync(rootPath + "/$/$.json", JSON.stringify(module.json, null, 2))
    }
    ...
    for(const folder of fs.readdirSync(rootPath)){
        ...
        if(!fs.existsSync(rootPath + "/" + folder + "/component")){
            fs.mkdirSync(rootPath + "/" + folder + "/component")
        }
        ...
        if(!fs.existsSync(rootPath + "/" + folder + "/" + folder + ".html")){
            fs.writeFileSync(rootPath + "/" + folder + "/" + folder + ".html", fs.readFileSync(root.models.page))
        }
            ...
            for await(const file of fs.readdirSync(dir)){
                if(fs.lstatSync(dir + "/" + file).isDirectory())await pathFindingJson(dir + "/" + file)
                    ...
                    if(!json.pageRender[name]){
                        json.pageRender[name] = {pageTitle: "page name"}
        ...
        async function pathFindingComponents(dir){
            for await(const file of fs.readdirSync(dir)){
                if(fs.lstatSync(dir + "/" + file).isDirectory())await pathFindingComponents(dir + "/" + file)
                    ...
                    let name = ((dir + "/").replace(rootPath + "/" + folder + "/component/", "") + file.replace(".html", "")).replace(/\//g, ".")
                    console.log("|-->" + name);
        ...
        await pathFindingComponents(rootPath + "/" + folder + "/component")
        ...
        async function pathFindingGet(dir){
            for await(const file of fs.readdirSync(dir)){
                if(fs.lstatSync(dir + "/" + file).isDirectory())await pathFindingGet(dir + "/" + file)
                    ...
                    if(fs.readFileSync(dir + "/" + file) == "")fs.writeFileSync(dir + "/" + file, fs.readFileSync(root.models.page))
                    let name = dir.replace(rootPath + "/" + folder + "/get", "") + "/" + file.replace(".html", "")
                    ...
                    if(!file.startsWith(":")){
                        ...
                        let pathHttpAcces = rootPath + "/" + folder + "/httpAcces" + name + ".mjs"
                        if(
                            fs.existsSync(pathHttpAcces) && 
                            fs.readFileSync(pathHttpAcces) == ""
                        ) fs.writeFileSync(pathHttpAcces, fs.readFileSync(__dirname + "/models/sub-httpAcces.js"))
