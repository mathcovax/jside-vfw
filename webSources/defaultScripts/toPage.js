class tp{
    constructor(newUrl, oldUrl=window.location.href){
        ...
        this.constructor.#timeoutLoading = setTimeout(() => {
            this.constructor.elementLoadingOverlay.dataset.vieujsProcess = "tp"
    ...
    static async #upDatePage(rep, newUrl, oldUrl){
        ...
        if(this.elementLoadingOverlay.dataset.vieujsProcess == "tp")this.elementLoadingOverlay.style.display = "none"
        this.#status = false
    }

    static #rectiNewPage(){
        for(const tag of this.#newPage.head.querySelectorAll("[data-vieujs-default], [data-vieujs]")){
            tag.remove()
        }
        for(const tag of this.#newPage.body.querySelectorAll("[data-vieujs-default], [data-vieujs]")){
            tag.remove()
        }
        for(const tag of this.#newPage.querySelectorAll("style[data-vieujs-page-script]")){
            let script = this.#newPage.createElement("script")
            if(tag.dataset.vieujsPageScript != ""){
                script.src = tag.dataset.vieujsPageScript
            }
            else{
                script.textContent = tag.textContent
            }
            script.dataset.vieujsPage = ""
    ...
    static #head(newUrl, oldUrl){
        ...
        for(const tag of this.#newPage.head.querySelectorAll("[data-vieujs-page]:not(link[rel='stylesheet'])")){
            tag.dataset.vieujsPage = newUrl
            this.elementHead.appendChild(tag)
        }
        for(const tag of this.elementHead.querySelectorAll(":not([data-vieujs-page='"+ newUrl +"']):not([data-vieujs-default]):not([data-vieujs])")){
            tag.remove()
        }
    }
    ...
    static async #loadCss(newUrl){
        ...
        for await(const tag of this.#newPage.querySelectorAll("style")){
            if(this.elementCss.querySelector("style[data-style-id='" + tag.dataset.styleId + "'][data-vieujs-page='"+ newUrl +"']")){
                tag.remove()
                continue
            }
            tag.dataset.vieujsPage = newUrl
    ...
    static #body(){
        for(let index = 1; this.elementBody.children[index] || this.#newPage.body.children[0]; index++){
            if(this.#newPage.body.children[0]?.dataset?.c && this.elementBody.children[index]?.dataset?.c && this.elementBody.children[index].dataset.c == this.#newPage.body.children[0].dataset.c){
                if(this.elementBody.children[index].dataset.vieujsScoped != this.#newPage.body.children[0].dataset.vieujsScoped){
                    this.elementBody.children[index].dataset.vieujsScoped = this.#newPage.body.children[0].dataset.vieujsScoped
                    for(const tag of this.elementBody.children[index].querySelectorAll("[data-vieujs-scoped]")){
                        tag.dataset.vieujsScoped = this.#newPage.body.children[0].dataset.vieujsScoped
                    }
                }
        ...
        this.elementBody.dataset.vieujsScoped = this.#newPage.body.dataset.vieujsScoped
    }
    ...
    static async #loadScripts(newUrl, oldUrl){
        ...
        for await(const script of document.querySelectorAll("script:not([data-vieujs]):not([data-vieujs-default])")){
    ...
    static async #unloadCss(newUrl){
        for(const css of this.elementCss.querySelectorAll("link:not([data-vieujs-page='"+ newUrl +"'])")){
            css.remove()
        }
        for(const style of this.elementCss.querySelectorAll("style:not([data-vieujs-page='"+ newUrl +"'])")){
            style.remove()
        }
    }
    ...
    static async #httpAcces(newUrl){
        return await new Promise((resolve, reject) => {
            fetch(newUrl, { method: "GET", headers: { "vieujs": "acces" } }).then(response=>{
    ...
    static async #getPage(newUrl){
        return await new Promise((resolve, reject) => {
            fetch(newUrl, { method: "GET", headers: { "vieujs": true } }).then(response=>{
    ...
    static on(event, fnc){
        ...
        else if(document.currentScript.dataset.vieujsDefault != undefined){
...
window.onload = async () => {
    ...
    for(const tag of document.querySelectorAll("[data-vieujs-page]")){
        tag.remove()
    }

    tp.elementVieujs = document.body.children[0]
    tp.elementBody = document.body
    tp.elementCss = tp.elementVieujs.querySelector("#css")
    tp.elementLoadingOverlay = tp.elementVieujs.querySelector("#loading-overlay")
    tp.elementHead = document.head

    let div = document.createElement("div")
    div.style.display = "none"
    div.id = "import-body"
    div.innerHTML = tp.elementVieujs.querySelector("#import-body").contentWindow.document.body.innerHTML
    tp.elementVieujs.querySelector("#import-body").replaceWith(div)
    for await(const ns of tp.elementVieujs.querySelector("#import-body").querySelectorAll("style[type='script']")){
        await new Promise((resolve) => {
            let s = document.createElement("script")
            if(ns.dataset.vieujsPageScript != ""){
                s.src = ns.dataset.vieujsPageScript
                s.onload = resolve
            }
            else{
